...
## Checkpoints — say these out loud, don't just do them

- **After each ticket lands**, state plainly: committed, merged into `develop`, pushed — and that
  it's a good point to clear context. Doing it silently is not the same as reporting it.
- **Relay the session nudge.** A user-level `Stop` hook reports context size, uncommitted work and
  unpushed commits. When it says context is in the `hard` band, tell the owner rather than carrying
