...
// The proxy layer is not a security boundary — the session gate and the ownership
...
describe('an unauthenticated visitor asking for a page', () => {
  it('is sent to sign-in', () => {
    expect(locationOf(proxy(request('/')))).toBe('/sign-in?next=%2F');
  });
  ...
  it('keeps where they were going, query string included', () => {
    ...
    expect(locationOf(proxy(request('/attempt/abc?q=1'))))
...
describe('an unauthenticated API call', () => {
  it('gets JSON, not a redirect', async () => {
    const response = proxy(request('/api/attempt/abc'));
  ...
  it('allows an API call once a session cookie is present', () => {
    expect(proxy(request('/api/attempt/abc', { cookie: true })).status).toBe(200);
  });
});
...
describe('the sign-in page is always reachable', () => {
  ...
  // the recovery move. The proxy sees the cookie; the server sees no session
  // and sends the visitor to sign-in. If the proxy bounced them back, the two
  ...
  it('does not redirect away from sign-in when a stale cookie is present', () => {
    const response = proxy(request('/sign-in', { cookie: true }));
  ...
  it('leaves the denied state reachable', () => {
    expect(proxy(request('/sign-in?denied=1', { cookie: true })).headers.get('location'))
      .toBeNull();
  });
  ...
  it('never sends a visitor somewhere that would send them onwards', () => {
    for (const path of ['/', '/exams', '/attempt/abc', '/sign-in']) {
      for (const cookie of [true, false]) {
        const first = proxy(request(path, { cookie }));
        const target = locationOf(first);
        if (target === null) continue;

        const second = proxy(request(target, { cookie }));
...
describe('a visitor with a session cookie', () => {
  it('is allowed through to a page', () => {
    expect(proxy(request('/', { cookie: true })).headers.get('location')).toBeNull();
  });
});
