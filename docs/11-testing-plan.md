...
| `select.ts` — domain mode | `20` / `40` / `all` each return exactly that, and `all` never exceeds the domain's non-holdout exam and recall pools. A practice and a domain sitting serve a `recall` item and never a holdout one — asserted in `selection.test.ts` inside a transaction its own sentinel rolls back (#62). |
...
| `sentry-options.test.ts` | The options the Sentry SDK is handed: on exactly when a DSN is set (which is what keeps it off a laptop), `sendDefaultPii` false, no traces, **no integrations at all** — the shape of the assertion that keeps Session Replay out, since replay arrives only by adding one — and the tested scrubber on **both** `beforeSend` and `beforeBreadcrumb`, by identity rather than equivalence, so a second scrubber written inline in provider configuration fails here. |
| `deploy-config.test.ts` | Every script in `app/package.json` that passes an env file uses the **conditional** form, derived from the manifest rather than from a list of script names. `.github/workflows/ci.yml` runs the three database-free suites, runs none of the three that write or read a database, holds no repository secret at all, and **pins a Node major strictly above the manifest's floor major** — so every version that pin can resolve to satisfies `engines.node`, whatever the runner picks. And `app/vercel.json` builds with `next build` alone, sets `git.deploymentEnabled` to a **branch map rather than the boolean that would also stop `main`**, denies by default, and grants **exactly one** exception — the last of those being an assertion whose only job is to refuse a second `true`. And `.github/workflows/deploy.yml` triggers on a push to `main` alone, runs the bank checks before `db:migrate` before `seed`, references **only** `DATABASE_URL_UNPOOLED` and only after `npm ci`, queues rather than cancels, and pins the same Node major as `ci.yml`. And `.github/dependabot.yml` watches `app/` and the workflows' actions and opens **every** update against `develop`, because a pull request merged into `main` is a deploy. And `app/src/proxy.ts` excludes the tunnel route `app/next.config.ts` actually mounts — two files that must agree about one string, where drift is silent: the proxy would redirect an unauthenticated browser's error report to the sign-in page, and nothing would throw. |
