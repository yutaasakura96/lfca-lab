...
- **`npm run check-bank` exists from Task 12 and is green only from Task 58.** It cannot be a green gate before then: with no questions written it reports 537 `q-concept-coverage` errors and exits 1, exactly as `check-guide` reported 537 missing concepts throughout cycle 2's writing. From Task 12 the per-task gate is the **scoped, suppressed** form in the Authoring Task Protocol, which must be clean for the competency being worked on. The unscoped, unsuppressed run becomes a hard gate at Task 58 and stays one.
- **The `npm test` counts stated in Tasks 5–13 are indicative, not targets.** They assume every test in this plan lands exactly as written. The requirement is that the suite passes and grows — never delete or weaken a test to reach a number in this document. If a count does not match, report the actual and move on.
