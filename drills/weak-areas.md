...
**Answer: A.** On systems running systemd-resolved the `hosts:` line commonly includes `resolve`, so lookups may be served by a local stub at 127.0.0.53 rather than by the nameserver a naive reading of `/etc/resolv.conf` suggests, which is exactly the kind of source `getent hosts` reveals and `dig` does not.
